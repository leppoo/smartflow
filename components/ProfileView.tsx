  ...
  return (
    <div className="max-w-3xl mx-auto pb-24 sm:pb-0 relative">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-primary-900">User Profile</h1>
          <p className="text-primary-400">Your professional business identity.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-primary-100 overflow-hidden mb-8">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label htmlFor="profileName" className="text-xs font-bold text-primary-400 uppercase">Your Name / Business Name</label>
                <input
                  id="profileName"
                  name="name"
                  value={formData.name}
                  ...
                  className="w-full rounded-xl border-primary-200 focus:ring-primary-500 focus:border-primary-500 p-3"
                />
              </div>
              <div className="space-y-1">
                <label htmlFor="profileEmail" className="text-xs font-bold text-primary-400 uppercase">Your Email</label>
                <input
                  id="profileEmail"
                  name="email"
                  type="email"
            ...
            <div className="space-y-1">
              <label htmlFor="profileAddress" className="text-xs font-bold text-primary-400 uppercase">Business Address</label>
              <textarea
                id="profileAddress"
                name="address"
            ...
            {!hasFilledBanks ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {profile.banks.filter(b => b.bankName.trim() !== '' || b.accountNo.trim() !== '').map((bank) => (
                  <div key={bank.id} className="bg-primary-50 p-4 rounded-2xl border border-primary-100">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[10px] font-black text-primary-400 uppercase tracking-widest">{bank.paymentMethod}</span>
                    </div>
                    <p className="font-bold text-primary-900 uppercase text-sm truncate">{bank.bankName || 'Unnamed Bank'}</p>
                    <p className="text-xs text-primary-400 font-mono truncate">{bank.accountNo || 'No Account Number'}</p>
                    <p className="text-[10px] text-primary-300 uppercase mt-2 truncate font-medium">{bank.accountName}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="p-6 bg-primary-50 border-t border-primary-100 flex justify-end gap-3">
          <button
            onClick={onBack}
            className="px-6 py-2 rounded-xl text-primary-600 font-bold hover:bg-white transition-colors"
          >
            Cancel
          </button>
          <button
      ...
      {!isComplete && (
        <div className="bg-primary-100 rounded-2xl p-4 flex items-center gap-3 text-primary-400 text-xs">
