...
import { formatCurrency, createNewLiability } from '../utils/invoice';
...
export const EditLiabilitiesView: React.FC<Props> = ({ financialData, onSave, onBack }) => {
  ...
  const addEntry = () => {
    setLiabilities(prev => [...prev, createNewLiability()]);
  };
  ...
  const totalRemaining = liabilities.reduce((sum, l) => sum + (l.remainingBalance || 0), 0);
    ...
    <div className="max-w-3xl mx-auto pb-24 sm:pb-0 relative">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-black text-primary-900">Liabilities</h1>
          <p className="text-primary-400 text-sm">Remaining: {formatCurrency(totalRemaining, currency)}</p>
        </div>
      </div>
