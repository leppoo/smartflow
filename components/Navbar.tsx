  ...
  return (
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50 no-print">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between max-w-6xl">
        <button 
          onClick={onHome}
          className="flex items-center gap-2 font-bold text-xl text-indigo-600 group"
        >
        ...
        <div className="flex items-center gap-4">
          <button 
            onClick={onProfile}
            className="text-slate-500 hover:text-indigo-600 text-sm font-bold transition-colors hidden sm:block bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100"
          >
            {displayName}
          </button>
          <button 
            onClick={onProfile}
            className="h-10 w-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 transition-all"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
